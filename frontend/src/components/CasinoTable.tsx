...
const CasinoTable = () => {
  ...
  const totalPot = engineState?.bets
    ? Object.values(engineState.bets).reduce((sum, value) => sum + value, 0)
    : 0;
  ...
  return (
    <div className="relative w-full h-full min-h-screen bg-[#0a0a14] overflow-hidden flex items-center justify-center p-4 font-[family-name:var(--font-geist-sans)]">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,#16302f_0%,#0a0a14_60%,#05050a_100%)]" />
      ...
      <div className="relative w-[95%] max-w-6xl aspect-[2.2/1] bg-gradient-to-br from-green-900 to-green-950 rounded-[240px] border-8 border-[#3d2b1f] shadow-[0_0_150px_rgba(0,0,0,0.9),inset_0_0_80px_rgba(0,0,0,0.6)] flex items-center justify-center">
        ...
        {players.map((player, idx) => (
          <div
            key={player.id}
            className={`absolute ${positions[idx] ?? positions[0]} p-4 flex flex-col items-center gap-2 transform scale-90 sm:scale-100 z-10`}
          >
            <div
              className={`w-20 h-20 rounded-full border-4 ${
                player.id === engineState?.currentPlayerId
                  ? 'border-yellow-400 ring-8 ring-yellow-400/20'
                  : 'border-white/10'
              } bg-gray-900 flex items-center justify-center text-sm font-black tracking-[0.3em] text-white shadow-2xl relative`}
            >
              {player.isBot ? 'BOT' : 'SEAT'}
              {player.isReady && gameState === 'LOBBY' && (
                <div className="absolute -bottom-2 px-2 bg-green-500 rounded text-[10px] text-white font-black uppercase">
                  READY
                </div>
              )}
            </div>
        ...
        <div className="flex flex-col items-center gap-8">
          <AnimatePresence>
            {engineState?.bets && (
              ...
              >
                <span className="text-[10px] text-yellow-500/80 font-black uppercase tracking-[0.2em]">
                  Total Stakes
                </span>
            ...
            <div className="bg-[#1a1a2e] p-8 rounded-[40px] border border-white/10 shadow-2xl flex flex-col items-center gap-6 max-w-sm w-full">
              ...
              <div className="flex items-center gap-4 bg-white/5 p-4 rounded-3xl border border-white/10 w-full justify-between">
                <button
                  ...
                  className="w-12 h-12 rounded-2xl bg-white/10 hover:bg-white/20 text-white font-bold transition-all"
                >
                  -
                </button>
