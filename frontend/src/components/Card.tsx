...
const Card = ({ card, index = 0, isFolded = false, onClick, isSelected = false }: CardProps) => {
  ...
  return (
    ...
    >
      <div className={`card-inner ${isFolded ? 'card-flip' : ''} w-24 h-36 border-2 ${isSelected ? 'border-yellow-400 ring-4 ring-yellow-400/20' : 'border-black/5'} rounded-xl shadow-xl cursor-pointer select-none`}>
        {/* Front Face */}
        <div className={`card-face bg-white rounded-xl p-2 flex flex-col justify-between ${getSuitColor(card.suit)}`}>
